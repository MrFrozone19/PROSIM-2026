...
/**
 * Capa de controles de la ventana AR: datos del equipo, dock de seis acciones, paneles y efectos.
 * Va encima del modelo (el 3D real en /scan o el placeholder en /ar) y deja pasar los toques al modelo:
 * solo los botones y paneles capturan eventos. La animación y el giro del modelo los ejecuta quien la hospeda.
 */
@Component({
  selector: 'app-ar-hud',
  imports: [Icon, StatCard, NgTemplateOutlet],
  host: { class: 'pointer-events-none absolute inset-0 block' },
  template: `
    <!-- partículas de celebración -->
    @if (effectsOn()) {
      <div class="absolute left-1/2 top-[46%] h-0 w-0">
    ...
    <div class="absolute inset-x-0 top-0 flex items-center justify-between px-5 pt-5">
      <button
        type="button"
        (click)="back.emit()"
        class="pressable pointer-events-auto flex h-10 w-10 items-center justify-center rounded-full border border-line bg-black/[0.67] text-white"
        aria-label="Regresar"
      >
      ...
      <div class="flex items-center gap-1.5 rounded-row border border-pink bg-pink/25 px-3 py-1.5">
        ...
        <span class="font-sans text-[11px] font-bold leading-normal text-white">{{ status() }}</span>
      </div>
    </div>
    ...
    <div class="absolute left-6 right-6 top-[74px] flex flex-col gap-1">
      ...
      @if (hint(); as h) {
        <p class="mt-1 w-fit rounded-pill border border-line bg-black/60 px-2.5 py-1 font-sans text-[10px] font-semibold text-white/85">
          {{ h }}
        </p>
      }
    </div>
    ...
    <ng-template #dockBtn let-item>
      <button
        ...
        class="pressable pointer-events-auto flex w-[54px] flex-col items-center gap-1.5"
      >
        <span
        ...
        <span
          class="whitespace-nowrap font-sans text-[11px] font-semibold leading-normal [text-shadow:0_1px_3px_rgba(0,0,0,0.9)]"
          [class]="isActive(item.id) ? 'text-white' : 'text-white/75'"
          >{{ item.label }}</span
        >
      </button>
    </ng-template>
    ...
    <!-- panel inferior -->
    @if (panel(); as p) {
      <div class="pointer-events-auto absolute inset-0 bg-black/40" (click)="closePanel()" aria-hidden="true"></div>
      <section
        class="sheet-in pointer-events-auto absolute inset-x-0 bottom-0 max-h-[72%] overflow-y-auto rounded-t-[32px] border-t border-line bg-surface/95 px-5 pb-6 pt-3 shadow-[0_-8px_24px_rgba(0,0,0,0.5)] backdrop-blur"
        role="dialog"
...
export class ArHud implements OnDestroy {
  readonly team = input.required<Team>();
  /** Estado real de la animación del modelo; el botón del dock alterna entre animar y detener. */
  ...
  protected readonly bottomDock = computed<DockItem[]>(() => [
  ...
  protected press(id: DockId): void {
    ...
    switch (id) {
      ...
      case 'info':
        ...
        if (this.panel() === 'info') this.infoOpened.emit();
        break;
      case 'video':
      case 'stats':
      case 'trivia':
